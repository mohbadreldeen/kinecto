...
const updateSegmentSchema = z
    .object({
        ...
        filterCriteria: segmentFilterSchema.optional(),
    })
...
export async function PATCH(
    ...
    const parsedBody = updateSegmentSchema.safeParse(body);
    ...
    const existing = await db.query.segments.findFirst({
        where: and(
            eq(segments.id, parsedParams.data.id),
            eq(segments.tenantId, tenantResult.context.tenantId)
        ),
    });
    ...
    const nextFilterCriteria =
        parsedBody.data.filterCriteria ??
        (existing.filterCriteria as z.infer<typeof segmentFilterSchema>);

    const where = buildCustomerWhereForSegment({
        tenantId: tenantResult.context.tenantId,
        filter: nextFilterCriteria,
    });
