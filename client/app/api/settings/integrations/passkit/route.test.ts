...
beforeEach(() => {
    mockUser = null;
    process.env.APP_ENCRYPTION_KEY = "test-encryption-key-for-passkit-settings";
});
...
describe("/api/settings/integrations/passkit", () => {
    it("saves and returns tenant passkit settings", async () => {
        const tenant = await createTenantOwner({
            tenantName: "Tenant PassKit Settings",
        });
        ...
        const saveRequest = new NextRequest(
            "http://localhost:3000/api/settings/integrations/passkit",
            {
                method: "PUT",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({
                    label: "Primary",
                    baseUrl: "https://passkit.example.com",
        ...
        expect(getBody.data.baseUrl).toBe("https://passkit.example.com");
        expect(getBody.data.templateId).toBe("tmpl_001");
        expect(getBody.data.apiKeyMasked).toBe("****7890");
    });

    it("returns configured false when no passkit key exists", async () => {
        const tenant = await createTenantOwner({
            tenantName: "Tenant Empty PassKit",
        });
