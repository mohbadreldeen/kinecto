...
type StoredEmailCredentials = {
    provider: "resend" | "sendgrid";
    apiKey: string;
    fromEmail: string;
};

function parseStoredEmailCredentials(rawValue: string) {
    ...
    if (!parsed.success) {
        throw new Error("Stored Email credentials are invalid");
    }
...
export async function GET() {
    ...
    const credentials = parseStoredEmailCredentials(row.encryptedKey);
...
export async function PUT(request: NextRequest) {
    ...
    const parsedBody = emailSettingsSchema.safeParse(body);
    ...
    const stored: StoredEmailCredentials = {
